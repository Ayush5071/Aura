...
const HistoryRequests = () => {
  ...
  console.log(profile,"histoy wale section me")

  return (
    <div className="p-6 bg-zinc-900 shadow-lg rounded-lg">
      <h2 className="text-2xl font-semibold text-zinc-100 mb-6">History of Completed Requests</h2>

      {profile?.historyRequests?.length > 0 ? (
        <div className="space-y-6">
          {profile.historyRequests.map((request) => (
        ...
        <p className="text-zinc-400">No tasks completed in history.</p>
      )}
    </div>
  );
};
